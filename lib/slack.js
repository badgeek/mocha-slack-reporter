    ...
    function Slack(runner, reporter_options) {
      ...
      runner.once("end", () => {
        ...
        if (failed) {
          ...
          messageOptions.text = options.failEmoji + " FAILED: " + buildSuiteName() + " [Failed: " + test_failed_total + "] [Success: " + test_passed_total + "] [Total Test: "+test_total+"]";
          ...
          messageOptions.text = options.passEmoji + " PASSED: " + buildSuiteName() + ` [Total Test: ${test_total}]`;
          // messageOptions.attachments.push({
          //   color: "#008000",
          //   text: "Passed all " + (test_failed.length+test_passed.length) + " tests",
          // });

        }
        
        if(process.env.CI) {
            messageOptions.text = messageOptions.text + " <" + process.env.CI_JOB_URL + "|(logs)> " + " <" + process.env.CI_PROJECT_URL + "/commit/" +  process.env.CI_COMMIT_SHA + "|(commit)> " + `<${process.env.CI_JOB_URL}/artifacts/download|(reports)>\n`;
