  ...
  const Slack = (function() {
    function Slack(runner, reporter_options) {
      ...
      runner.once("end", () => {
        ...
        if(process.env.CI) {
            messageOptions.text = messageOptions.text + " <" + process.env.CI_JOB_URL + "|(logs)> " + " <" + process.env.CI_PROJECT_URL + "/commit/" +  process.env.CI_COMMIT_SHA + "|(commit)> " + `<${process.env.CI_JOB_URL}/retry|(retry)>\n`;
